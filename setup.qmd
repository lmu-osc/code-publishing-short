...
Throughout this tutorial, you will need to run both R code and system commands (primarily `git` and `quarto`). Within RStudio, R code can be run by going to the tab _Console_, while system commands are executed in the tab _Terminal_. We also indicate where to run your code directly above each code snippet.
...
Luckily, there are already numerous proposals for how to organize one's project files, both general [e.g., @Wilson2017; @TIER4] as well as specific to a particular programming language [e.g., @Marwick2018; @Vuorre2021; @ArayaSalas2024] or journal [@Vilhuber2021]. We recommend you to follow the standards of your field.
