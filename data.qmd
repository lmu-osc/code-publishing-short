...
When distributing a data set, it is important to document the meaning (e.g., units)
and valid values of its variables.
...
using the R package [`pointblank`][pointblank].
...
You can put the code that follows for creating the data dictionary
into a new file called `create_data_dictionary.R`.

First, we write down everything we know about the data set.
This includes:
...
As the data set is from the R package `palmerpenguins`,
if you have it installed you can use the function `citation()`
...
Finally, we can store the data dictionary inside an HTML file
and put the HTML file into the project folder as well.

```{.r filename="create_data_dictionary.R"}
pointblank::export_report(dict, filename = "data_dictionary.html")
```

For a more elaborate introduction to `pointblank`,
you can read its "[Intro to Information Management][pointblank-intro]".

[pointblank-intro]: https://rstudio.github.io/pointblank/articles/INFO-1.html
...
As before, we also provide a reference to the source.
