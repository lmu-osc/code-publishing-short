...
using the R packages [`datawizard`][datawizard] and [`tinytable`][tinytable].
...
Next, you can use `datawizard` again to create
the data dictionary containing the descriptions,
but also some other information about each variable
(e.g., the number of missing values).
Finally, you can use `tinytable` to print the table.
...
(dict <- datawizard::data_codebook(dat) |>
  subset(select = -c(ID, .row_id)) |>
  tinytable::tt() |>
  tinytable::format_tt(replace = "")) # hide NAs
```
