...
Every expression of an idea, such as literary or artistic works, are automatically covered by copyright. This means that nobody else than the copyright holder is allowed to copy, modify, or share it. Copyright licenses can grant others some of the necessary rights, but mostly don't cover publicity, privacy, moral, patent, or trademark rights.
...
1. Recording the copyright notices and licenses for files by others (for example, in a file called `LICENSE.txt`) and following all their additional requirements.
2. In the simplest case, choosing two licenses (one for code, one for everything else) which to apply  simultaneously (at the choice of the recipient) to all own files (for example, again by recording the license in the file `LICENSE.txt`).
...
The answer is that you need to indicate the license on a per-file or per-folder basis (rather than choosing one for the whole project). The easiest approach is to make note of every foreign work included in your project and record its license. Then, dual-license all the remaining files, which are yours, under a code and a non-code license in parallel as explained in @tip-license-r.
