...
In most cases, the output of software, like images or tables, does not depend on its license. Therefore, if you use an R package under a copyleft license to create content, you are likely the copyright owner. However, if the output is based in data, it can be considered a derivative work and the license of the data also applies.

If a Quarto document contains both R code and writing, one may wonder what type of license to apply. It is possible to make such a file simultaneously available under two licenses, at the choice of the recipient: Either under a specified software license, or under a Creative Commons license^[that is, a license for writing, image, audio, and video]. This is called dual-licensing and makes it easier to reuse both rendered PDF as well as the code. For example, one could write:

> The Quarto files in this project are made available under CC\ BY-SA\ 4.0, or (at your option) under the terms of the Apache Software License\ 2.0.
...
all_deps <- tools::package_dependencies(
  ...
  append(deps) |>
  unique()
unique(installed.packages(fields = "License")[pkgs, "License"])
```
:::
