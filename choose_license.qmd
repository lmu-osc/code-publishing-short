...
2. In the simplest case, choosing two licenses (one for code, one for everything else) which to apply  simultaneously to all own files (for example, again by recording the license in the file `LICENSE.txt`). Recipients are then free to choose under which of the two licenses to use the material.
...
> Except otherwise noted, all files in this project are made available under CC0\ 1.0 or (at your option) under the terms of the Apache Software License\ 2.0.
