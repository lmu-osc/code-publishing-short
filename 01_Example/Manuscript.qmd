...
dat_desc$sex[is.na(dat_desc$sex)] <- "unknown" # Convert NA to "unknown"
dat_desc$year <- as.factor(dat_desc$year) # Convert year to factor

# Add number of non-missings to names of numeric variables
n <- sapply(dat_desc, \(v) sum(!is.na(v))) # Calculate number of non-missings
labels <- paste0(names(dat_desc), " (N=", n, ")") # Paste variable names and numbers together
idx <- which(sapply(dat_desc, is.numeric)) # Identify numeric variables
names(dat_desc)[idx] <- labels[idx] # Change the variable names
