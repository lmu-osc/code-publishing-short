...
options(modelsummary_warning_latex_siunitx_preamble = FALSE) # Hide LaTeX compilation tips
modelsummary::datasummary_balance(formula = ~ species, data = data_clean, output = "latex", dinm = FALSE)
...
Female penguins ($M = `{r} round(mean(data$bill_length_mm[data$sex == "female"], na.rm = TRUE), digits = 2)` \mathrm{mm}, SD = `{r} round(sd(data$bill_length_mm[data$sex == "female"], na.rm = TRUE), digits = 2)` \mathrm{mm}$) exhibited shorter bill lengths compared to male penguins ($M = `{r} round(mean(data$bill_length_mm[data$sex == "male"], na.rm = TRUE), digits = 2)` \mathrm{mm}, SD = `{r} round(sd(data$bill_length_mm[data$sex == "male"], na.rm = TRUE), digits = 2)` \mathrm{mm}$).
